...
CREATE OR REPLACE VIEW syndicat.vw_historic AS 
         SELECT operation.o_id AS id, operation.o_date AS xdate, operation.o_label AS label, ( SELECT sum(operation_detail.od_amount) AS sum
		  ...
		  mp_description ,
		    (select string_agg(po_name, ',' order by po_name) from poste_analytique where po_id in (select po_id from syndicat.operation_detail where o_id=operation.o_id) )as activite
